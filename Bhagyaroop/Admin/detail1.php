    ...
    <body>
              ...
              <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <div class="action">
                  <div class="myForm">
                      ...
                      <div class="document">
                        <?php
                        $updatedPeople = "SELECT * FROM `updated` WHERE `email`= '$email' ";
                        $updatedPeopleExecute = mysqli_query($con,$updatedPeople);
                        $updatedPeopleRows = mysqli_num_rows($updatedPeopleExecute);
                        for($i=1; $i<=$updatedPeopleRows; $i++){
                            $data = mysqli_fetch_assoc($updatedPeopleExecute);
                            $CP = $data['CP'];
                            $UA = $data['UA'];
                            $UI = $data['UI'];
                            $UE = $data['UE'];
                            ...
                            <form action="" method="POST">
                                <input type="hidden" name="update" value="abc">
                                <ul>
                                    <?php
                                    if($UE != ""){
                                        ?>
      ...
      if(isset($_POST['file']) ){
        ...
        if(preg_match("/\.(pdf)$/", $uploadedFile)){
          ?>
          <script>
            let url = "./../Bhagyaroop/user/documents/<?php echo $uploadedFile; ?>";
            var importantStuff = window.open(url, '_blank').focus();
          </script>
          <?php
        }else{
          ?>
      ...
      if(isset($_POST['update'])){
          ...
          $updateVerification = mysqli_query($con,"UPDATE `verify` SET `file1`='$file1',`file3`='$file3',`file4`='$file4' WHERE `email`='$email' "); 
          $updateData = mysqli_query($con," UPDATE `bhagyaroop_signup` SET `education_field`='$field1', `income`='$field2', `address`='$field3' WHERE `semail` = '$email' ");
          if($updateData == true){
              $deleteRecord = mysqli_query($con,"DELETE FROM `updated` WHERE `email`='$email' ");
